...
#Non je ne pense pas cela soit bon 
#We can see that the prior and the posterior distribution of K are not completely the same. However, the convergence has been reached for K and it's look good. 
...
```
It's not really realistic because the apparent calf production is very low. 
...
data.frame(posterior_gamr = posterior_gamr %>%
   ...
   ggtitle("r")
```
# SAMUEL # kezako We observe that the posterior distribution of r has changed and the distribution is much more 
...
data_constant <- list(n_obs = gray %>% 
 ...
 data_stochastic <- list(COUNT = gray %>% pull(abundance_estimate), CATCH = gray %>% 
                        pull(catch))
